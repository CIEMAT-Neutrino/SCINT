...
def average_wvf(my_runs,PATH="../data/ave/"):
    ...
    try:
        ana_runs = load_analysis_npy(my_runs["N_runs"],my_runs["N_channels"],-1)
    except:
    ...
    for run,ch in product(my_runs["N_runs"],my_runs["N_channels"]):
        # print(run,ch)
        my_runs[run][ch]["ADC"] = ana_runs["P_channels"]*np.mean((my_runs[run][ch]["ADC"].T-ana_runs[run][ch]["Ped_mean"]).T,axis=0)
