...
def custom_plotly_layout(
    fig_px, xaxis_title="", yaxis_title="", title="", barmode="stack", bargap=0
):
    ...
    fig_px.update_layout(
        ...
        legend_title_text="",
        legend=dict(yanchor="top", xanchor="right", x=0.99),
        showlegend=True,
    )
...
def vis_event(in_file):
    ...
    def update_plot(column):
        ...
        if column in df.columns.to_list():
            ...
            fig.update_layout(
                title="Raw Waveform - Event %i" % column,
                xaxis_title="Time [s]",
                yaxis_title="ADC",
            )
            return fig
