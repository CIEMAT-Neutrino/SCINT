...
def save_fit_parameters(run, ch, m_fit, fit_function, variable, info, user_input):
    ...
    header = [m_fit.parameters[int(idx / 2)].upper() if idx % 2 == 0 else "D" + m_fit.parameters[
        int(idx / 2)].upper() for idx in range(len(m_fit.values) * 2)]
    write_output_file(run, ch, output=parameters, filename=fit_function + variable, header_list=header, info=info,
                    not_saved=[], debug=user_input["debug"])
...
def plot_minuit_fit(m_fit, xdata, ydata, labels, user_input, info, OPT):
    ...
    plt.axvline(xdata[int(len(xdata) / 2)], color='white', label="\n".join(legend), alpha=0)
