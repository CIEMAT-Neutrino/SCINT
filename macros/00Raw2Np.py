...
### 00Raw2Np
binary2npy(np.asarray(user_input["runs"]).astype(int), np.asarray(user_input["channels"]).astype(int), user_input=user_input, compressed=True, force=True, debug=user_input["debug"])