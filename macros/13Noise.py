import sys; sys.path.insert(0, '../'); from lib import *
default_dict = {"channels":["CHAN_TOTAL"], "runs":["NOISE_RUNS"]}
user_input = initialize_macro("13Noise",["input_file","runs","key","load_preset","filter","debug"],default_dict=default_dict, debug=True)
info = read_input_file(user_input["input_file"][0], debug=user_input["debug"])

### 11Average
my_runs = load_npy(np.asarray(user_input["runs"]).astype(int), np.asarray(user_input["channels"]).astype(int), info, preset=user_input["load_preset"][0], compressed=True, debug=user_input["debug"])
compute_fft_wvfs(my_runs,info=info,key=user_input["key"][0],label=user_input["label"][0],debug=False)

save_proccesed_variables(my_runs, preset="FFT", info=info, force=True, debug=user_input["debug"])
del my_runs
gc.collect()