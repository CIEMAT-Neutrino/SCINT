...
for run, ch in product(np.asarray(user_input["runs"]).astype(str),np.asarray(user_input["channels"]).astype(str)):
    ...
    popt, pcov, perr = calibrate(my_runs, info, [user_input["variables"][0]], OPT, save=user_input["save"], debug=user_input["debug"])
    calibration_txt(run, ch, popt, pcov, filename="Calibration", info=info, debug=user_input["debug"])