...
for run, ch in product(my_run["NRun"],my_run["NChannel"]):
    ...
    for filter_key in ["Raw","Gauss","Wiener"]:
        if filter_key == "Raw": color = "tab:blue"
        if filter_key == "Gauss": color = "tab:green"
        if filter_key == "Wiener": color = "tab:orange"
        ...
        if filter_key == "Raw":
            ax[0].plot(1e6*my_run[run][ch]["Sampling"]*np.arange(len(my_run[run][ch]["McNoiseADC"][num])),my_run[run][ch]["McNoiseADC"][num],c="tab:blue",label="MC NOISE",alpha=.75)

        else:
            ax[0].plot(1e6*my_run[run][ch]["Sampling"]*np.arange(len(my_run[run][ch][filter_key+dec_key][num])),my_run[run][ch][filter_key+dec_key][num],c=color,label="MC NOISE")

        for i in range(1,len(ax)):
            if i == 1 or i == 2 :
                n,bins,patches = ax[i].hist(my_run[run][ch][key_value[i-1]],bins=30,alpha=0.75,color=color)
                ...
                n,bins,patches = ax[i].hist(my_run[run][ch][key_value[i-1]],bins=np.linspace(0,np.max(my_run[run][ch][key_value[i-1]]),30),alpha=0.75,color=color)
                ax[i].axvline(bins[np.argmax(n)]+bins[1]/2,c="grey",ls=":",label="MODE: %.2f"%(bins[np.argmax(n)]+bins[1]/2))
                ax[i].axvline(np.mean(my_run[run][ch][key_value[i-1]]),c="grey",label="MEAN: %.2f"%np.mean(my_run[run][ch][key_value[i-1]]))
                ax[i].axvline(np.mean(my_run[run][ch][key_value[i-1]])+np.std(my_run[run][ch][key_value[i-1]]),c="grey",ls="--",label="STD: %.2f"%np.std(my_run[run][ch][key_value[i-1]]))
                ax[i].axvline(np.mean(my_run[run][ch][key_value[i-1]])-np.std(my_run[run][ch][key_value[i-1]]),c="grey",ls="--",label="STD: %.2f"%np.std(my_run[run][ch][key_value[i-1]]))
