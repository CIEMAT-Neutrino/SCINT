...
user_input = initialize_macro("02Process",["input_file","debug"],default_dict=default_dict, debug=True)
...
for run, ch in product(np.asarray(user_input["runs"]).astype(int),np.asarray(user_input["channels"]).astype(int)):
    ...
    compute_pedestal_variables(my_runs, label="Ana", key="AnaADC", ped_lim=500, debug=user_input["debug"]) # Compute new ped variables using sliding window
