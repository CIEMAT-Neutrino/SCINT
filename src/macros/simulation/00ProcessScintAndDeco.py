...
my_run["NChannel"] = []
template_list = [
    "{root}/data/SC_Test/raw/wvf_config_1.npy",
    "{root}/data/SC_Test/raw/wvf_config_2.npy",
    "{root}/data/SC_Test/raw/wvf_config_3.npy",
]
...
for i in range(len(template_list)):
    ...
    for j in np.linspace(500, 1000, 6).astype(str):
        ...
        my_run[i + 1][j]["SPEChargeADC"] = my_run[i + 1][j]["Sampling"] * np.sum(
            10 * SER[i][i_idx:f_base_idx] / np.max(SER[i])
        )
        ...
        if np.size(SER[i]) > my_run[i + 1][j]["TimeWindow"]:
            ...
            SER[i] = np.concatenate(
                (SER[i], np.zeros(my_run[i + 1][j]["TimeWindow"] - np.size(SER[i])))
            )
        ...
        my_run[i + 1][j]["GaussCutOff"] = (
            my_run[i + 1][j]["TimeWindow"] * cutoff * my_run[i + 1][j]["Sampling"]
        )
