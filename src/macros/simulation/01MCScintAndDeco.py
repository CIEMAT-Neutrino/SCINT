...
for run, ch in product(my_run["NRun"], my_run["NChannel"]):
    buffer = (
        4 * my_run[run][ch]["TimeWindow"]
    )  # Buffer expanding array to accomodate scint wvfs [ticks]
    my_run[run][ch]["McADC"] = []
    my_run[run][ch]["Mct0"] = []
    for jj in range(MC_num):
        times = rand_scint_times(
            100, fast=6e-9, slow=1.4e-6, ratio=0.7
        )  # Main mc function generates photon arrival times
        wvf = np.zeros(buffer)
        for t in times:
            this_peak = int(
                (t - (t % my_run[run][ch]["Sampling"])) / my_run[run][ch]["Sampling"]
            )  # Calculate the modulus of the arrival time according to sampling.
            this_wvf = np.zeros(my_run[run][ch]["PreTrigger"] + this_peak)
            this_wvf = np.append(this_wvf, my_run[run][ch]["SPE"][0])
            this_wvf = np.append(
                this_wvf, np.zeros(len(wvf) - len(this_wvf))
            )  # If this line gives errors, make buffer bigger
            wvf = wvf + this_wvf
    ...
    my_run[run][ch]["McNoiseADC"] = np.asarray(
        my_run[run][ch]["McADC"]
    ) + np.random.normal(
        0,
        np.max(my_run[run][ch]["SPE"]) * 0.5,
        size=[len(my_run[run][ch]["McADC"]), len(my_run[run][ch]["McADC"][0])],
    )
    ...
    for j in range(len(raw_key)):
        ...
        for i in range(len(my_run[run][ch][raw_key[j]])):
            ...
            time = (
                ...
                * (
                    np.argmax(my_run[run][ch][raw_key[j]][i])
                    - my_run[run][ch]["PreTrigger"]
                    + 1
                )
                - my_run[run][ch]["Mct0"][i]
            )
            noiseSTD = np.std(my_run[run][ch][raw_key[j]][i][:i_base_idx])

            if raw_key[j] == "McNoiseADC":
                charge = my_run[run][ch]["Sampling"] * np.sum(
                    my_run[run][ch][raw_key[j]][i][i_base_idx:f_base_idx]
                )
                my_run[run][ch][wvf_key[j] + "Charge"].append(
                    charge / my_run[run][ch]["SPEChargeADC"]
                )

            else:
                charge = np.sum(
                    my_run[run][ch][raw_key[j]][i][
                        my_run[run][ch]["PreTrigger"]
                        - 20 : my_run[run][ch]["PreTrigger"]
                        + 300
                    ]
                )
        ...
        my_run[run][ch][wvf_key[j] + "Charge"] = np.asarray(
            my_run[run][ch][wvf_key[j] + "Charge"]
        )
        my_run[run][ch][wvf_key[j] + "Amp"] = np.asarray(
            my_run[run][ch][wvf_key[j] + "Amp"]
        )
        my_run[run][ch][wvf_key[j] + "Time"] = np.asarray(
            my_run[run][ch][wvf_key[j] + "Time"]
        )
        my_run[run][ch][wvf_key[j] + "NoiseSTD"] = np.asarray(
            my_run[run][ch][wvf_key[j] + "NoiseSTD"]
        )
